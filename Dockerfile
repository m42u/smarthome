...
RUN apt-get update
RUN apt-get install -y curl git python make build-essential libavahi-compat-libdnssd-dev avahi-daemon && rm -rf /var/lib/apt/lists/* && apt-get clean
...
RUN npm install -g homebridge homebridge-lifx-lan homebridge-rcswitch-gpiomem
